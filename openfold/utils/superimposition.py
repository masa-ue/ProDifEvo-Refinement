...
def _superimpose_single(reference, coords):
    ...
    rmsd_list = np.sum((reference_np - superimposed) * (reference_np - superimposed),1)
    return coords.new_tensor(superimposed), coords.new_tensor(rmsd), rot, tran, rmsd_list
...
def superimpose(reference, coords, mask, return_transform=False):
    ...
    for r, c, m in zip(flat_reference, flat_coords, flat_mask):
        ...
        superimposed, rmsd, rot, tran, rmsd_list = _superimpose_single(
            r_unmasked_coords, 
            c_unmasked_coords
        )
        rots.append(rot)
    ...
    return superimposed_reshaped, rmsds_reshaped, rmsds_list_stacked
